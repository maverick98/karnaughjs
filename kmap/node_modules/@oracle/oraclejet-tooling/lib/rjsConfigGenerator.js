...
function _getExchangeCcaPathMapping(buildType, requirejs) {
  let pathMappingObj = {};
  const { components: exchangeComponents } = util.readJsonAndReturnObject(`./${CONSTANTS.ORACLE_JET_CONFIG_JSON}`);
  if (!exchangeComponents) return pathMappingObj;
  Object.keys(exchangeComponents).forEach((exchangeComponent) => {
    const exchangeComponentJson = util.readJsonAndReturnObject(path.join(
